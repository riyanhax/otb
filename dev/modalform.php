...
// Валидация формы
if ( !empty($_POST["modal-email"]) && !empty($_POST["modal-password"]) ) {
    $mess .= 'Email: '.clean( $_POST['modal-email'] ).' <br>';
    $mess .= 'Пароль: '.clean( $_POST['modal-password'] ).' <br>';
    $mess .= 'Телефон: '.clean( $_POST['modal-tel'] ).' <br>';
    $mess .= 'Имя: '.clean( $_POST['modal-name'] ).' <br>';
